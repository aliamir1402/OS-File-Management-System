...
int main()
{
	...
	Sleep(100);//Adding a 0.1 second delay, so that no user-functions are busy
	std::thread t2(&thread_function);
	Sleep(100);//Adding a 0.1 second delay, so that no user-functions are busy
	std::thread t3(&thread_function);
	t1.join();   // main thread waits for the thread t1 to finish
	t2.join();   // main thread waits for the thread t2 to finish
	t3.join();  // main thread waits for the thread t3 to finish
